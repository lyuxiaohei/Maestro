...
本文档定义产研工作流的 5 条铁律门禁规则。这些规则由 workflow 编排器在阶段推进过程中强制执行，不可绕过。
...
#### 证据要求检查（源自 devkit verification-gate）

6.5. 关键产出决策必须引用上游 STATE.md 或 CONTEXT.md 的具体段落作为证据。无证据支撑的决策（如"根据分析得出..."但无引用）标记为 INSUFFICIENT。证据引用格式: "依据 STATE.md P05-需求分析 > 关键发现 #3: ...": 防跳步规则
...
#### 证据支撑验证（源自 devkit verification-gate）

9.5. 检查产出中的每个关键决策是否有上游证据支撑。上游证据来源: STATE.md / CONTEXT.md / PLAN.md 的具体段落。无证据支撑的决策在验证报告中标记为 INSUFFICIENT。INSUFFICIENT 项超过 3 个: 验证结果降级为 CONDITIONAL。

---

## 附录: 证据先于断言范式（源自 devkit verification-gate）

### 门禁函数（5 步）

声称任何状态之前: (1) 确认证明命令 → (2) 执行 → (3) 阅读输出 → (4) 验证 → (5) 做出声称。跳过任何一步 = 撒谎，不是验证。
