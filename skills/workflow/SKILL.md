...
GATE-03 通过后、规划流水线前，对需要讨论的阶段执行讨论：
...
1. **spawn phase-planner**（调度域 Agent，见 `references/agent-contracts.md`）— 传入 phase_index、skill_name、upstream_outputs、phase_dir
...
3. **spawn research-synthesizer**（调度域 Agent，见 `references/agent-contracts.md`）— 传入所有 researcher 输出路径
...
| `../discuss/SKILL.md` | 讨论技能（灰色区域识别 + 交互决策） |

## Skill 名称映射

> 安装后所有 skill 以 `maestro-` 为前缀注册到 `~/.claude/skills/`，调度时使用完整名称。

| 目录名 | 注册名称 | 覆盖阶段 |
|--------|---------|---------|
| meeting-minutes | maestro-meeting-minutes | P01, P03 |
| diagram-design | maestro-diagram-design | P02 |
| competitive-analysis | maestro-competitive-analysis | P04 |
| feature-list | maestro-feature-list | P05 |
| logic-list-spec | maestro-logic-list-spec | P06, P09 |
| prototype-design | maestro-prototype-design | P06 |
| prototype-review | maestro-prototype-review | P07 |
| ui-design | maestro-ui-design | P08 |
| architecture-design | maestro-architecture-design | P10 |
| architecture-review | maestro-architecture-review | P11 |
| architecture-refinement | maestro-architecture-refinement | P12 |
| detailed-design | maestro-detailed-design | P13 |
| dev-task-planner | maestro-dev-task-planner | P14 |
| frontend-dev | maestro-frontend-dev | P15 |
| backend-dev | maestro-backend-dev | P15 |
| code-review | maestro-code-review | P15 |
| test-engineering | maestro-test-engineering | P16 |
| training-materials | maestro-training-materials | P16 |
| acceptance-testing | maestro-acceptance-testing | P17 |
| deployment | maestro-deployment | P18 |
| discuss | maestro-discuss | 全阶段（按需） |
| prd-auto-generator | maestro-prd-auto-generator | P09+ |
